...
class KderpWebsite(http.Controller):
	...
	def kdvn_file_library(self, post_id, page, pager_url, template=['kdvn_website_theme.kdvn_theme_files', 'website.homepage']):
		"""
	...
	def kdvn_image_library(self, page=1):
		...
		return self.kdvn_file_library(post.id, page, '/kdvn/images', 'kdvn_website_theme.kdvn_theme_files')
	...
	def kdvn_download(self, page=1):
		...
		return self.kdvn_file_library(post.id, page, '/intro/download','kdvn_website_theme.kdvn_theme_files')
		
	#Su dung chung cho cac trang News, Electrical va Mechanical
	post_limit = 9	
	def kdvn_posts(self, blog_name_list=[], post_ids=[], page_url='/', page=1, template=['kdvn_website_theme.kdvn_theme_news_post', 'kdvn_website_theme.kdvn_theme_news_list_posts']):
	...
	def kdvn_intro_post(self, post=None, **kw):
		...
		return http.request.render('kdvn_website_theme.kdvn_theme_intro_kdvn',{'posts':posts, 'posts_detail':posts_detail})
	...
	def kdvn_certificates(self, **kw):
		...
		return http.request.render('kdvn_website_theme.kdvn_theme_certificates', {'posts': posts})
	...
	def kdvn_contacts(self, **kw):
		...
		return http.request.render('kdvn_website_theme.kdvn_theme_contacts',{
			'offices': offices,
			'sites': sites
		})
	...
	def termofuse(self, **kw):
		...
		return http.request.render('kdvn_website_theme.kdvn_theme_termofuse', {'posts': posts})
	...
	def kdvn_prj_completed(self,page=1,url="/prj/completed", filter=[],template='',type='all',area='all', year='all', category='all',**searches):
		...
		return self.send_prj(
							...
							template='kdvn_website_theme.kdvn_theme_prj_completed', 
